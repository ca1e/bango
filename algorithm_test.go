...
`)
	x, y := s.run(8, 0)
	if x != 3 || y != 7 {
		t.Fatalf("move = (%d,%d), want the forced block at (3,7)", x, y)
	}
}

// TestOpeningPriorEnvOff: BANGO_OPENPRIOR=0 restores the raw search choice
// (the remote diagonal-two at depth 8), pinning the switch's effect.
func TestOpeningPriorEnvOff(t *testing.T) {
	t.Setenv("BANGO_OPENPRIOR", "0")
	s := diagramToSearcher(t, `
...............
...............
...............
...
`)
	x, y := s.run(8, 0)
	if x != 5 || y != 5 {
		t.Fatalf("prior off: move = (%d,%d), want the raw search choice (5,5)", x, y)
	}
}
