...
│   └── random/        #   random 算法：空盘天元；前一手杀局判断（跳五/冲四成五点、
│                      #   活三四两端加权随机）；否则在全部子的切比雪夫距离<3 邻域内
│                      #   取距前一切比雪夫距离最小的点，仅平局时按权重随机
...
**random 算法决策流**：① 空盘直接返回**天元**；② **杀局判断**——沿前一手四个方向滑动 5 格窗口，「同一方 4 子 + 有且仅有一个空位」即跳连成五点（含冲四/活四的成五点），堵唯一空点（多个窗口命中时按距前一手加权随机）；连珠长 3/4 且两端皆空判**活三/活四**，在两端点中按距前一手远近加权随机选一；③ 常规落子：收集距棋盘**所有**子切比雪夫距离 < 3 的空白交点，取其中距**前一手**切比雪夫距离**最小**的点，仅当最小距离点多于一个时在组内加权随机（同距权重相同，即均匀随机；无前一手信息时以距最近棋子的距离为排序键）。
...
[`vcx.go`](algorithm/alphabeta/vcx.go) 实现教程第八章的「算杀」：只考虑连续威胁（冲四/活三）的证明搜索。正常迭代加深没有找到必胜解时，先做 12 层 VCF（仅冲四），再做 10 层 VCT（冲四+活三），各占一步预算的 1/4：
