...
│                      #   组合根——import 两个算法包完成注册，BANGO_ALGO 选择算法
├── engine.go          # 引擎核心：状态管理、棋盘操作、前一手(最近落子)追踪、输出控制；
│                      #   只依赖 algorithm.Algorithm 接口，aiMove 把棋盘快照为 Request 后委托 Think
├── protocol.go        # 协议数据结构：Command 定义与坐标解析
├── engine_ai_test.go  # 引擎×算法集成测试：aiMove 合法性、开局库协议端到端
...
├── algorithm/         # ★ 算法集：统一接口 + 注册表 + 各算法实现
│   ├── algorithm.go   #   Algorithm 接口（Name/Think/Reset/EndSession）、Request 上下文、
│   │                  #   注册表（Register/New/Names，默认 alphabeta，BANGO_ALGO 可选）
│   ├── alphabeta/     #   原 alphabeta 搜索算法（原根目录 algorithm/rules/opening/tt/vcx/book*.go）
│   │   ├── adapter.go       # 算法适配器：置换表/开局库生命周期，Think=原 aiMove
│   │   ├── search.go        # 搜索核心：评估（按线缓存）、negamax+αβ+PVS、quiescence、
│   │   │                    #   迭代加深、杀手/历史启发、着法生成
│   │   ├── rules.go         # 规则判定：胜负分派与连珠禁手检测
│   │   ├── opening.go       # 经典 26 式开局：模式枚举、识别、理论区投影
│   │   ├── tt.go            # 置换表：Zobrist 哈希与缓存读写
│   │   ├── vcx.go           # 算杀：VCF/VCT 连续威胁搜索
│   │   ├── bookcmd.go       # 开局库维护子命令：validate / build（RunBookCommand）
│   │   ├── swap2.go         # SWAP2 开局决策（评估黑方形势→换先或执白回手）
│   │   └── *_test.go        # 搜索/规则/置换表/算杀/开局库/时间控制/自对弈等全套测试
│   └── random/        #   random 算法：空盘天元；己方成五点优先（自胜检查）；前一手杀局
│                      #   判断（跳五/冲四成五点、活三四两端加权随机、跳活三取空隙）；否则在
│                      #   全部子的切比雪夫距离<3 邻域内取距前一切比雪夫距离最小的点，
│                      #   仅平局时按权重随机
├── book/              # ★ 开局库功能模块：数据源解析、8 对称规范化编译、查询（引擎与算法共用）
...
> 分层约定：根目录只保留协议会话层（package main），算法一律放 `algorithm/` 下的子包并通过
> `algorithm.Algorithm` 接口接入；`book/` 是无搜索依赖的纯数据模块，供会话层、算法与工具共用。
...
- [`resetBoard()`](engine.go)：重置/分配指定大小的空棋盘，并调用 `algo.Reset` 让算法的跨手缓存（置换表）跟随对局生命周期
- [`place()`](engine.go)：在合法坐标处落子（越界自动忽略），同时维护**前一手（最近落子）锚点**，供算法做威胁分析与邻近加权；TAKEBACK 撤掉锚点子时自动失效
- [`aiMove()`](engine.go)：计算落子位置——持锁快照棋盘/规则/时限/前一手为 `algorithm.Request`，解锁后委托 `algo.Think`，返回盘外或满盘时兜底首个空点
...
### 5.0 算法集与统一接口（`algorithm/`）

落子决策被抽象为 [`algorithm.Algorithm`](algorithm/algorithm.go) 接口，会话层（`package main`）只依赖接口：

```go
type Algorithm interface {
    Name() string                            // 注册名："alphabeta"、"random"
    Think(req Request) (x, y int)            // 一步决策：Request 是棋盘快照+规则+时限+前一手
    Reset(size int, maxMemory int64)         // 新对局/换盘面：兼容的置换表等缓存可跨 RESTART 存活
    EndSession()                             // 会话结束：丢弃全部跨对局状态
}
```

- **Request**：`Size` + 行主序棋盘（0 空 / 1 己方 / 2 对手 / 3 标记）+ `OwnIsBlack` + 规则码 + 时限/深度/内存预算 + INFO folder + **前一手坐标 `LastX/LastY`**（`HasLast=false` 表示空盘）。
- **注册表**：各算法子包在 `init()` 中 `algorithm.Register(name, factory)`；`main.go` import 即注册。运行时用环境变量 **`BANGO_ALGO`** 选择（默认 `alphabeta`），未知名字在 stderr 告警后回退默认。
- **新增算法**：建 `algorithm/<name>/` 子包实现接口并注册，不改动引擎核心。

内置算法：

| 算法 | 路径 | 说明 |
| --- | --- | --- |
| `alphabeta` | [`algorithm/alphabeta/`](algorithm/alphabeta/) | 完整搜索 AI（5.1-5.9），`adapter.go` 持有置换表与开局库生命周期 |
| `random` | [`algorithm/random/`](algorithm/random/) | 加权随机基线，带前一手杀局判断（见下） |

**random 算法决策流**：① 空盘直接返回**天元**；② **自胜检查（两层搜索·第一层）**——己方已有四（冲四/活四/跳四）时立即取成五点取胜，优先级高于一切防守；③ **杀局判断**——沿前一手四个方向滑动 5 格窗口，「同一方 4 子 + 有且仅有一个空位」即跳连成五点（含冲四/活四的成五点），堵唯一空点（多个窗口命中时按距前一手加权随机）；连珠长 3/4 且两端皆空判**活三/活四**，在两端点中按距前一手远近加权随机选一；**跳活三**（6 格窗口内 3 子 + 1 内部空隙、两外侧皆空）由专门检查识别，直接取空隙点，把跳型威胁拦在升级成跳四之前（两层搜索·第二层，优先于跳四阶段的防守）；④ 常规落子：收集距棋盘**所有**子切比雪夫距离 < 3 的空白交点，取其中距**前一手**切比雪夫距离**最小**的点，仅当最小距离点多于一个时在组内加权随机（同距权重相同，即均匀随机；无前一手信息时以距最近棋子的距离为排序键）。

### 5.1 棋盘表示与胜负判定（alphabeta）

搜索在棋盘的扁平副本 `b[y*size+x]`（0 空 / 1 己方 / 2 对手）上进行，落子/撤子即写入/清零。胜负判定由 [`winsMove()`](algorithm/alphabeta/rules.go) 按当前规则分派（见 5.8 节）。

### 5.2 着法生成

[`genMoves()`](algorithm/alphabeta/search.go:268) 只考虑与已有棋子切比雪夫距离 ≤ 2 的空点，避免全盘枚举导致分支爆炸。

**候选数据增量化**：落子/提子唯一原语 `setStone` 在评估线缓存之外再维护三份增量状态——子数计数器、半径 2 / 半径 1 两张邻域引用计数表（取代每空格的 5×5 / 3×3 扫描）、以及**逐格逐向棋型值缓存**：`pointScore` 的单方向体（双向数子 + 开端 + 跳形折半）按 (格 × 4 向 × 2 方) 缓存，`setStone` 对 4 个方向上 ±9 步依赖窗口内的格子打脏标记（按方向×方共 8 位），读取时惰性重算。正确性门槛：[`TestShapeCacheExact`](algorithm/alphabeta/shape_cache_test.go) 在随机 make/undo 序列上逐步断言缓存值与独立 oracle 逐位相等（含子数与两张邻域表）。搜索树与逐空格扫描版本逐位一致。

**威胁梯子双保留不变量（2026-08 修复「活三不防」）**：[`genMovesRanked()`](algorithm/alphabeta/search.go) 的每一级威胁梯子都同时保留**双方**的格子——行棋方自己的获胜点在前、对方的紧随其后（参考实现 eval.js `bySide`/`orderedSet` 同款结构）。成五级合并 `fivesMe+fivesOpp`、活四级合并 `liveFourMe+liveFourOpp` 并让双方冲四随行；单边提前返回只在引擎行棋节点语义成立（"我方活四下一手必胜"），放到对方行棋节点会把对方自己的反击点整体隐藏——搜索据此构造出幻影必胜分（例如对方节点候选只剩挡点时，己方活四威胁被评成 `winScore−6`），根节点于是放弃挡对方的活三转头"进攻"（`TestGenMovesKeepsOpponentCounterThreats` / `TestGenMovesKeepsOpponentFive` / `TestAnswersLiveThreeWithOwnFourThreat` 回归）。附带收益：活四级随行对方的冲四点，`x.ooo` 跳四的缺口封堵点重新进入候选集。

### 5.3 静态局面评分（评估函数）

[`evaluate()`](algorithm/alphabeta/search.go:404) 逐行扫描全部行、列、对角线，由 [`scoreLineFor()`](algorithm/alphabeta/search.go:452) 识别棋型并累计分值，最终评分 = 己方总分 − 对方总分。评分表：
...
全盘扫描的结果按线缓存（见 5.14）：落子/撤子只重扫穿过的 4 条线，`evaluate()` 本身 O(1) 运行总和；[`evaluateFull()`](algorithm/alphabeta/search.go) 保留原扫描逻辑作为位级对照基准。
...
[`pointScore()`](algorithm/alphabeta/search.go:330) 对每个候选点做快速的「落子后四方向棋型」预估，以 进攻分 + 防守分 降序排序。好的排序让 Alpha-Beta 剪枝接近最优效率，是搜索深度达到 6~8 层的关键。

### 5.6 迭代加深与时间控制

[`run()`](algorithm/alphabeta/search.go:109) 从深度 2 逐层加深，上一轮最佳着法在下一轮优先搜索；预算由 `INFO timeout_turn` / `time_left`（毫秒）换算（默认 1 秒，取 `timeout_turn` 的 90%，并保留整局时钟 3/4 的余量），超时的层直接放弃、沿用上一层结果。**根节点 aspiration 窗口**：以上一层分值 ±3 万（一个活三量级，按局面波动自适应放大）为窗口搜索下一层，落窗外围绕观测边界几何加宽重搜——最终重搜保证根分值与全窗逐位一致，仅在分值平稳时省节点。15 路中局实测：深度 12 累计 ~0.55s，**深度 14 于 ~2.8s 健全完成**（`maxSearchDepth` 上限即 14）。

### 5.7 置换表与 Zobrist 哈希

不同的走法顺序可能到达同一局面（"置换"，如先下 A 再下 B ≡ 先下 B 再下 A）。[`tt.go`](algorithm/alphabeta/tt.go) 为每个 (盘面, 行棋方) 节点维护 64 位 Zobrist 指纹，缓存搜索结果：
...
- **验收测试**：固定深度下开启/关闭置换表必须得到完全相同的根分值与着法（[`tt_test.go`](algorithm/alphabeta/tt_test.go) `TestTTSearchEquivalence`），同深度 8 节点数降至原来的 ~83%，迭代加深整流程耗时约减半。

### 5.8 规则体系与禁手（INFO rule）

[`rules.go`](algorithm/alphabeta/rules.go) 按 `INFO rule` 代码实现各规则的胜负判定与连珠禁手：
...
- **胜负判定**统一走 [`winsMove()`](algorithm/alphabeta/rules.go)：搜索、算杀、战术捷径全部按当前规则判定，不再使用固定的 `>=5`。
...
[`vcx.go`](algorithm/alphabeta/vcx.go) 实现教程第八章的「算杀」：只考虑连续威胁（冲四/活三）的证明搜索。正常迭代加深没有找到必胜解时，先做 12 层 VCF（仅冲四），再做 10 层 VCT（冲四+活三）。**时间预算（2026-09 修复）**：杀棋搜索与主梯子共享**同一绝对截止时间**（`runSingle` 按整手预算设定，杀棋分得梯子用剩的部分），不再各阶段「从现在起」追加 1/4、1/8 切片——旧做法叠加后无上界，中盘强制树爆炸时一手可超预算十余倍（arena 实测 5.3s/预算 360ms，`TestThinkHoldsWholeMoveBudget` 回归）。超时语义：deadline 每节点检查、触发即置 `timedOut` 并沿递归立即上抛，超时的证明一律视为**未定**——不报告杀棋、更不得当作已验证的防守（`TestKillSearchPastDeadlineInconclusive` 回归）：

- **MAX 层（进攻方）**：只走己方的四（VCT 再加活三），任一分支成五即证明杀棋；
- **MIN 层（防守方）**：走双方所有的四/活三（防守含反击）**与挡成五点、挡活三的成活四点**，有一种防住即证明不成杀。防守列表按 quiescence `fourThreats` 同款双视角构建——若只列防守方自己的四/活三，「没有反四」会被误读成「无法防守」，单纯冲四（挡一格即化解）就被当成必胜返回；成五点入防守列表（key 略低于己方成五，有五先赢）。**2026-08 二次修复**：VCT 模式下防守层再加「对方活三的成活四点」（key=冲四−1）——否则进攻方把活二长成新活三时，该三尚无成五点可挡、防守集合为空，连一个裸活二都被「证明」必胜并绕过主搜索直接落子（`TestKillSearchLiveTwoNotAKill` / `TestKillSearchFreshThreeDefended` 回归，真杀测试 7/7 保全；修复后 arena 50 局 26:24 优胜）。活四仍不可防、含强制挡四的真杀链不受影响。**防守列表不截断（2026-09）**：`nodeMoveLimit=12` 的截断只作用于进攻层（漏杀可接受），防守列表被截断等于把成立的防守藏起来、凭空造杀（`TestDefenceForcingMovesUncapped` 回归）；
- **无假阳性**：搜索超出深度视作防守成功，绝不误报杀棋。
- **已证胜局护栏（2026-08）**：主搜索已证明己方必胜（`prevScore ≥ winScore−64`）时整个杀棋搜索跳过——只看连续威胁的窄证明不得用未证明的替代着法覆盖主搜索的全宽胜解。
- **对方杀棋探测（2026-08，参考实现 candidateMinmax 防线）**：己方 VCF/VCT 均未证明时，以对方为进攻方跑一次 VCT——若对方（假如下一手轮到它）存在连续威胁必胜，则**占据其主威胁点**，且落子后重探验证封锁确实瓦解证明才采纳（连珠下黑方引擎落封锁点前查禁手）。**2026-09 修复**：验证不再只试杀线首着一个点，而是枚举己方必答点（成五点封堵 + 对方活三成活四点，上限 6 个）逐个重探，找到任一瓦解证明的点即采纳；封不掉的多点叉威胁交回主搜索着法。超时一律视为未定：探测或验证中途超时都返回 -1，绝不用没搜完的搜索充当「已验证防守」。**触发门槛（2026-09）**：主梯子完成迭代且评分 ≥ 0（不认为有危险）时探测不得覆盖梯子着法——窄证明在安静/占优局面下噪声过大，实测天元主线探测的封堵 (8,6) 黑胜仅 14%、被它覆盖的梯子着法 (6,5) 黑胜 93%，门槛后天元局黑胜率 33.5% → 85.7%（`TestLadderMoveKeptWhenNoDanger` / `TestOppKillProbeBlocksOpponentKill` / `TestOppKillProbeLiveTwoNotAKill` / `TestOppKillProbeRejectsUnblockableFork` / `TestKillSearchPastDeadlineInconclusive` / `TestOppKillProbeBoundedOnOverrunPosition` 回归）。
...
- **展开集合**：[`fourThreats()`](algorithm/alphabeta/search.go) 列出双方的成四/成五点（活四两端、跳四缺口均由 `pointScore` 识别），双方同权、强者先行，连珠禁手照常过滤；深度上限 4 层（恰好「四→挡→反四→挡」一个来回）；
...
[`book/book.go`](book/book.go) 移植教程原生项目（gobang）的开局库模块，数据格式互通：
...
- **经典 26 式开局**（[`opening.go`](algorithm/alphabeta/opening.go)，2026-08）：黑1 天元 + 白1 直指/斜指 + 黑2 在天元 5×5 盒内的对称类，规范化去重后**恰好 13 直指 + 13 斜指 = 26 式**，与经典计数吻合。`book gen-classic` 枚举种子线，`book build --seeds` 以固定深度搜索生长续着（成五点终止行、绝不入书），产出 `openbook/classic-26-15x15.json`。**定位为模式引导书**（`modeBook`）：其续着来自深度 8 自搜索、弱于实战搜索，arena 实测直接跟走会输棋（11:19），故只作理论区来源、绝不直接采纳；直接采纳仍只用赛事级 gomocup 书
...
PVS 的依据是：主要变例通常只有一条，找到第一个能产生截断的走法后，其余走法大概率不比它好。[`minimax()`](algorithm/alphabeta/search.go) 与根节点 [`searchRoot()`](algorithm/alphabeta/search.go) 中：第一个走法全窗口搜索；其后走法先以零窗口（己方 (α, α+1)、对方 (β−1, β)）侦察，只证明该走法落在窗口外；侦察值仍落在 (α, β) 内才以全窗口重搜定值。`winsMove` 与叶子分支本就是精确值，不侦察不重搜。
...
- **历史启发**：按角色分表的 `history[2][n×n]`，截断时 `+= depth²`（越深的子树被截断越值得早试）；每次迭代加深前全表减半老化。应用范围严格分段：[`genMovesRanked()`](algorithm/alphabeta/search.go) 返回安静尾巴起点，成五/冲四/双三的威胁阶梯原序保留（`TestGenMovesPriorityLadder` 金丝雀），仅活三及更弱的尾巴按统计稳定重排，slot 0 永不重排。

实测（中局基准深度 12）：杀手走法把节点数砍半（73,717 → 35,296）；历史启发在杀手之后边际收益 ~0.5%（杀手指槽已覆盖大部分截断场景），保留因其成本近零、且安静尾巴更宽的实战局面中收益更大。

### 5.14 评估按线缓存

全盘 `6n−2` 条线（n 行、n 列、两族对角各 2n−1）各自缓存双方棋型分，`evalTotal` 为运行总和。落子/撤子统一走 [`setStone()`](algorithm/alphabeta/search.go)：增量 XOR 哈希 + 只重扫穿过该点的 4 条线（O(n)），`evaluate()` O(1) 返回。选线缓存而非教程的逐点缓存，是为了**位级等价**——评估语义一字不改，全部开/关等价测试自动成为回归覆盖。正确性门槛：[`TestEvalIncrementalExact`](algorithm/alphabeta/tt_test.go) 在 9/15 路、freestyle/renju 四种模式下随机走子+悔子 3,000 步，每步断言缓存总和与全盘扫描逐位相等。基准耗时再降 3~4%（杀手剪枝后叶节点占比已低；叶评估 O(1) 化为后续「位置加成」类改进提供增量维护的基础）。
...
- [`negamax_test.go`](algorithm/alphabeta/negamax_test.go)：13 组固定局面固定深度的根分值/着法黄金基线，保值搜索栈（LMR 关闭）必须逐位复现预置数值（根节点行棋方恒为引擎，分值语义不变）；
- [`algorithm_test.go`](algorithm/alphabeta/algorithm_test.go)：棋型评分表逐例校验；「一步成五必取」「对方四必挡」「活三必防」等战术断言；评估函数换边反对称校验；着法生成优先级阶梯（成五/活四级双方合并、行棋方在前，`TestGenMovesKeepsOpponentCounterThreats` 回归「活三不防」根因）、Chebyshev-2 候选窗的边界裁剪与去重、`quietFrom` 强制段/安静尾巴排序契约；门控 quiescence 的门控等价、五威胁识别、lastP 未知保守展开、双活四/遗留四的静态兜底（`TestQuiescenceGateQuietPosition`/`TestQuiescenceSeesFive`/`TestQuiescenceLeftoverFourFallsBackToStatic` 等）；
- [`tt_test.go`](algorithm/alphabeta/tt_test.go)：Zobrist 增量哈希一致性、置换表读写/替换策略/同槽异锁碰撞保护、固定深度与**整轮迭代加深**的开/关等价性、`max_memory` 表收缩、剪枝安全性（开/关 Alpha-Beta 结果必须一致）与提速统计；PVS、杀手走法、历史启发各自的**开/关等价性**与提速统计；TT 胜利分区间守卫的正/反对照（区外深度条目直接复用、区内绝不作 cutoff，`TestTTWinScoreGuard`）；评估线缓存的位级属性测试与线 id 编解码契约（`TestEvalIncrementalExact`/`TestLineGatherRoundTrip`），含悔棋-重放型序列的 `TestEvalIncrementalWithTakebacks`；
- [`shape_cache_test.go`](algorithm/alphabeta/shape_cache_test.go)：增量候选状态（子数/邻域引用计数/逐向棋型值缓存）的正确性门槛——随机 make/undo 序列逐步断言缓存与独立 oracle、计数器与全盘重扫逐位相等；
- [`time_test.go`](algorithm/alphabeta/time_test.go)：`thinkBudget` 预算公式表驱动校验；极小预算下战术捷径必命中、真实思考必返回合法着法；deadline 过期后 `run` 回退且**置换表零写入**；中止节点（入口/移循环中）不入表；小预算中断后续搜满深度必须收敛到全新搜索的同分同着（`TestInterruptedThenResumedConsistent`）；
- [`search_bench_test.go`](algorithm/alphabeta/search_bench_test.go)：中局/战术两个固定图面的确定性搜索基准（`go test -run '^$' -bench BenchmarkSearch -benchtime 1x -v`），输出节点数与截断数供优化对比；
- [`vcx_test.go`](algorithm/alphabeta/vcx_test.go)：双三/活三必胜杀棋必须找到、安静局面不得误报、对方冲四可破解杀棋、可挡冲四不得误报必杀、经过强制挡四的真杀链必须找到；
- [`rules_test.go`](algorithm/alphabeta/rules_test.go)：三三/四四/长连禁手、**四三合法**、跳四交叉 4-4、白棋无禁手、五子优先覆盖禁手、着法生成过滤（黑滤/白留）、各规则（0/1/4/8/9）胜负语义（含 caro 边缘端点、standard 长连不胜）、连珠自对弈完局；禁手枚举后增量评估/哈希不被扰动；
- [`book_test.go`](book/book_test.go) / [`book_engine_test.go`](algorithm/alphabeta/book_engine_test.go) / [`bookcmd_test.go`](algorithm/alphabeta/bookcmd_test.go)：开局库编译与对称查询、权重累积、首着平移、规则/尺寸护栏、禁手候选过滤、协议端到端命中、validate/build 闭环；**执白 freestyle 颜色映射采纳**、精确威胁门槛六例（安静/双方向跳二伪威胁/连三/跳三/x.x.x/成四点）、openbook 默认书路径与装载；
- [`ai_selfplay_test.go`](algorithm/alphabeta/ai_selfplay_test.go)：完整自对弈回归（合法性、能分出胜负）与搜索深度/节点数统计（`go test -v -run TestSearchStats` 查看）；
...
```bash
BANGO_ALGO=alphabeta ./pbrain-bango   # 默认：完整搜索 AI
BANGO_ALGO=random ./pbrain-bango      # 加权随机基线（带前一手杀局判断）
```

`BANGO_ALGO` 支持任意已注册算法名（`algorithm.Names()`：`alphabeta`、`random`）；未知名字在 stderr 告警后回退 `alphabeta`，引擎始终可对弈。
...
将编译产物 `pbrain-bango` 配置到 Gomocup 竞赛管理器（如 Piskvork）中即可参与对局。引擎会响应 `ABOUT` 命令返回，其中 `ai` 字段为当前选中的算法（`BANGO_ALGO`，默认 `alphabeta`）：

```text
name="pbrain-bango", version="0.1", author="cale && GLM5.3", ai="alphabeta"
```
